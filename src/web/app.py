...
from src.utils.webhook_store import load_webhooks, add_webhook, remove_webhook, get_active_webhooks, regenerate_secret
...
def create_app(config: dict) -> FastAPI:
    ...
    async def add_webhook_endpoint(data: dict = Body(...)):
        ...
        result = add_webhook(url, name)
        if result:
            return JSONResponse({"status": "ok", "message": "已添加", "secret": result["secret"]})
    ...
    @app.post("/api/settings/webhooks/regenerate")
    async def regenerate_webhook_secret(data: dict = Body(...)):
        url = data.get("url", "").strip()
        secret = regenerate_secret(url)
        if secret:
            return JSONResponse({"status": "ok", "secret": secret, "message": "密钥已重新生成"})
        return JSONResponse({"detail": "未找到该 Webhook"}, status_code=404)
